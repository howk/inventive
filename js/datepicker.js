  ...
  _updateUi() {
    ...
    do {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        cell.classList.add(ClassNames.TDCELL);
        span.classList.add(ClassNames.TABLE_CELL);
        ...
        svg.setAttributeNS(null, 'class', 'circle-icon');

        const circle = document.createElementNS(xmlns, "circle");
        circle.setAttributeNS(null, 'cx', '9.5');
        circle.setAttributeNS(null, 'cy', '9.5');
        circle.setAttributeNS(null, 'r', '8.5');
        circle.setAttributeNS(null, 'stroke', '#00BAC6');
        circle.setAttributeNS(null, 'class', 'circle-icon__line');
        ...
        if (date.getMonth() == this._month) {
          ...
          if (
            ...
            span.dataset.date = date.getDate();
...
const calendarElem = document.querySelectorAll('.calendar-sec');
if (calendarElem.length > 0) {
  calendarElem.forEach(calendarElemItem => {
    const today = new Date();
    const calendar = new Calendar(calendarElemItem, {
      minDate: new Date(
        today.getFullYear(),
        today.getMonth(),
        today.getDate()
    ...
    calendarElemItem.addEventListener(Events.DATE_SELECTED, (event) => {
